...
function parseURL(input, flag) {
	...
	if ( input.indexOf(siteURL) === 0 ) {
		console.log('already proxied'+input);
		return input;
	}
	...
	console.log('input to parseURL' + input);
	...
	/////$$$$
	//console.log('$$$$$$' + siteURL);
	//console.log('ginf.target.h: ' + ginf.target.h +'ginf.target.b: ' + ginf.target.b + 'ginf.target.p: ' +  ginf.target.p)
...
function parseHTML(html) {
	...
	if ( typeof(html) != 'string' ) {
	 ...
	 // Proxy an update to URL based attributes 先不管
	 ...
	 // Proxy iframe, ensuring the frame flag is added 先不管
	 ...
	 // Proxy attributes
	 parser = /\s(href|action)\s*=\s*(["']?)([^"'\s>]+)/ig;
	 while ( match = parser.exec(html) ) {
		  html = html.replace(match[0],' '+match[1]+'='+match[2]+parseURL(match[3]));
	 }

	 // special parse for background
	 parser = /\s(background)\s*=\s*(["']?)([^"'\s>]+)/ig;
	 while ( match = parser.exec(html) ) {
		  html = html.replace(match[0],' '+match[1]+'='+match[2]+parseURL(match[3]));
	 }


	 // special parse for src
	 parser = /<img[.*]?\s(src)\s*=\s*(["']?)([^"'\s>]+)/ig;
	 console.log('$$$$$$img src match!'+parser);
	 while ( match = parser.exec(html) ) {
	 	  console.log('$$$$$$img src match!');
		  html = html.replace(match[0],' '+match[1]+'='+match[2]+parseURL(match[3]));
	 }
...
document.write = function(html) {
	...
	console.log('@@@@write'+ginf.override);
...
document.writeln = function(html) {
	...
	console.log('@@@@writeln'+ginf.override);
	// Do want to interfere?
	if ( ginf.override || args[args.length-1] == 'gl' ) {
		//html = parseHTML(html);
	}
